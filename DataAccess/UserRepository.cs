...
namespace DataAccess
{
	public class UserRepository : BaseRepository<User>, IUserRepository
	{
		...
		public UserRepository(IMongoDbContext mongoDbContext, ISecurityService securityService)
			: base(mongoDbContext)
		{
		...
		public async Task<bool> AddUserTokenByUserIdAsync(string userId, Token token)
		{
			...
			{
				FilterDefinition<User> filter = new FilterDefinitionBuilder<User>().Eq(x => x.Id, userId);
				UpdateDefinition<User> update = new UpdateDefinitionBuilder<User>().AddToSet(
					x => x.Tokens,
					token
				);
		...
		public async Task<bool> DeleteExpiredTokensAsync(string userId)
		{
			try
			{
				...
				UpdateDefinition<User> update = new UpdateDefinitionBuilder<User>().PullFilter(
					x => x.Tokens,
					i => i.RefreshTokenExpiresDateTime < DateTimeOffset.UtcNow
				);
		...
		)
		{
			...
			try
			{
				...
				UpdateDefinition<User> update = new UpdateDefinitionBuilder<User>().PullFilter(
					x => x.Tokens,
					i =>
						i.RefreshTokenIdHashSource == refreshTokenIdHashSource
						|| (
							i.RefreshTokenIdHash == refreshTokenIdHashSource && i.RefreshTokenIdHashSource == null
						)
				);
		...
		)
		{
			try
			{
				...
				FilterDefinition<User> filter = new FilterDefinitionBuilder<User>().Eq(
					$"{nameof(User.Tokens)}.{nameof(Token.RefreshTokenIdHash)}",
					refreshTokenHash
				);
				...
				return (
					user.Tokens.Where(x => x.RefreshTokenIdHash == refreshTokenHash).FirstOrDefault(),
					user
				);
			}
			catch
		...
		)
		{
			UpdateDefinition<User> update = new UpdateDefinitionBuilder<User>()
				.Set(i => i.ProviderKey, newPasswordHash)
				.Set(x => x.SerialNumber, newSerialNumber);
		...
		public async Task<bool> SetEmailVerifiedAsync(string userId)
		{
			...
			{
				FilterDefinition<User> filter = new FilterDefinitionBuilder<User>().Eq(x => x.Id, userId);
				UpdateDefinition<User> update = new UpdateDefinitionBuilder<User>().Set(
					x => x.IsEmailVerified,
					true
				);
